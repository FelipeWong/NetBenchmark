...
def random_walk(network_tuple, args):
    ...
    if Spliting:  # based on node2vec https://github.com/lucashu1/link-prediction
        ...
        if not DIRECTED:
            ...
            adj_train, train_edges, train_edges_false, val_edges, val_edges_false, \
                test_edges, test_edges_false = mask_test_edges_directed(adj, test_frac=test_frac, val_frac=val_frac,
                                                                        prevent_disconnect=False,
                                                                        false_edge_sampling='random', seed=seed)
    ...
    g_n2v = Graph_walk(g_train, DIRECTED, P,
                       Q)  # create node2vec graph instance and P & Q follow the biasd par. p & q from node2vec
    ...
    output_k = ['cata_sentences', 'edges', 'gl', 'dict_NtoC', 'id2word', 'word2id', 'subsamples', 'sentences',
                'adj_train', 'feat', 'adj', 'nb_id']
    output = [cata_sentences, edges, gl, dict_NtoC, id2word, word2id, subsamples, sentences, adj_train, feat, adj,
              nb_id]
