...
def Scan_Context_process(PAR, args):
    ...
    np.random.seed(seed)
    Preindex = np.random.choice(ALL_Win[:, 0], nb_negative + 10, replace=False,
                                p=[(x / sum(ALL_Win[:, 1])) for x in ALL_Win[:, 1]])
    ...
    for i in range(1, num_node):
        ...
        while len(index) < nb_negative:
            x = int(
                np.random.choice(ALL_Win[:, 0], 1, replace=False, p=[(x / sum(ALL_Win[:, 1])) for x in ALL_Win[:, 1]]))
    ...
    out_name = ["mulDmatrix", "mulreformfn", "mulcount_list", "Max_win_count", "mulnegFre", "mulnegFrea",
                "mulComatrix_1hop"]
